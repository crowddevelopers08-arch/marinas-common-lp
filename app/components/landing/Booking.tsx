...
export function Booking() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("booking:reset", resetForm);
    return () => window.removeEventListener("booking:reset", resetForm);
  }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const params = new URLSearchParams({ name, phone, condition: cond });
  ...
  return (
    <section className={cx(section, "relative overflow-hidden bg-[#14241f] text-[#f2fbfc] after:absolute after:right-[-150px] after:top-[-120px] after:size-[420px] after:rounded-full after:border after:border-white/10 after:content-['']")}>
      <div className={cx(wrap, "relative grid grid-cols-2 gap-[46px] max-[980px]:grid-cols-1 max-[980px]:gap-9")}>
        ...
        <div id="booking" className="scroll-mt-[96px]">
          <Reveal>
            <div className={cx("rounded-3xl bg-white p-[34px] text-[#14241f] max-[760px]:p-[26px_22px]", cardShadow)}>
              <div>
                ...
                <form ref={formRef} onSubmit={submit} noValidate>
                  ...
                  <button type="submit" disabled={submitting} className={cx(button.primary, button.block, button.lg, "whitespace-nowrap max-[350px]:px-4 max-[350px]:py-3.5 max-[350px]:text-[14px]", submitting && "cursor-wait opacity-70")}>{submitting ? "Sending..." : "Request my consultation"}</button>
